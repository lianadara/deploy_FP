...
# Load model
model_filename = "model_next.pkl"
with open(model_filename, 'rb') as file:
    pipeline = pickle.load(file)

# Fungsi untuk melakukan prediksi
def predict_score(avg_review, jumlah_pasien, revenue, cogs):
    # Buat DataFrame dari input user
    profit = revenue - cogs
    input_data = pd.DataFrame({
        'lag_1_avg_review': [avg_review],
        'lag_1_jumlah_pasien': [jumlah_pasien],
        'lag_1_total_revenue': [revenue],
        'lag_1_total_profit': [profit]
    })
    print(input_data)
...
# Interface Streamlit
    ...
    if st.button('Predict Score'):
        # Lakukan prediksi
        predicted_score = predict_score(avg_review, jumlah_pasien, total_revenue, cogs)
